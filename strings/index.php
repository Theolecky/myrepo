...
//var_dump-->prints whatever is passed to it and showing the data and datatype

//count-->returns the array sign
...
<table border="1" cellspacing='10'>
  ...
  <?php
    for($i=0;$i<$columns;$i++){
  ?>
      <th><?php echo preg_replace("/[\+\&\#()=>0-9]/is" , "" ,
        ucwords(strtolower($wambua_corrupted_sample_array[$i])));?>
      </th>
...
<?php
  $words = explode(",",ucwords(strtolower($wambua_corrupted_sample)));
  $word3 = array_splice(array_unique($words), 8, 254);
  $array_of_rows = array_chunk($word3,8);
  // echo '<table border="1">';
